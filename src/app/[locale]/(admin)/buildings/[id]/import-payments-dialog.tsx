...
} from "./import-payments-actions";

const errorLabelKeys: Record<ParsedPaymentRow["errors"][number], string> = {
  missing_unit_number: "errorMissingUnitNumber",
  unit_not_found: "errorUnitNotFound",
  missing_amount: "errorMissingAmount",
  missing_date: "errorMissingDate",
...
export function ImportPaymentsDialog({
  ...
  const [rows, setRows] = useState<ParsedPaymentRow[] | null>(null);
  ...
  async function handleParse(formData: FormData) {
    setParsing(true);
    const result = await parsePaymentsImport(buildingId, formData);
    setParsing(false);
  ...
  async function handleConfirm() {
    if (!rows) return;
    setImporting(true);
    const result = await commitPaymentsImport(buildingId, tenantId, rows);
    setImporting(false);
        ...
        {!rows ? (
          <form action={handleParse} className="flex flex-col gap-4">
            <Button variant="link" className="w-fit px-0" asChild>
              <a href={`/${locale}/buildings/${buildingId}/payments-template`} download>
                <DownloadIcon />
                {t("downloadTemplate")}
              </a>
            </Button>
            <div className="grid gap-2">
              <Label htmlFor="payments-file">{t("importFileLabel")}</Label>
              <Input id="payments-file" name="file" type="file" accept=".xlsx" required />
            </div>
            <DialogFooter>
